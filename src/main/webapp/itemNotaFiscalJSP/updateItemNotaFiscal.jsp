...
<%
    // Lê o parâmetro 'id' e busca o item correspondente
    ...
    if (id != null) {
        item = new ItemNotaFiscalDAO().buscarPorId(id);
    }
%>
...
<form action="<%= request.getContextPath() %>/ServletUpdateItemNotaFiscal" method="post">
    ...
    <select id="idNotaFiscalXml" name="idNotaFiscalXml" required>
        ...
        <option value="<%= nf.getId() %>"
                <%= nf.getId().equals(item.getIdNotaFiscalXml()) ? "selected" : "" %>>
    ...
    <select id="idEmpresa" name="idEmpresa" required>
        ...
        <option value="<%= emp.getId() %>"
                <%= emp.getId().equals(item.getIdEmpresa()) ? "selected" : "" %>>
    ...
    <input type="number" step="0.01" id="quantidade" name="quantidade"
           value="<%= item.getQuantidade() %>" required>
    <br><br>

    <label for="preco">Preço Unitário:</label>
    <input type="number" step="0.01" id="preco" name="preco"
           value="<%= item.getPreco() %>" required>
    <br><br>
